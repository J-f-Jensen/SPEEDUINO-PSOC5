...
#if defined (SLC_OEM_WBO)
    #include "src/SLC_OEM_WBO/SLC_OEM_WBO.h"
#endif
...
{
#if defined (SLC_OEM_WBO)
  // Read AFR value from SLC_OEM using I2C
  uint8_t tempO2value = SLC_OEM_WBO_readAFR(SLC_OEM_WBO_I2C_ADDRESS), ADCFILTER_O2, currentStatus.O2

  if (tempO2value == 0xFF ) // WBO2 not ready - use limp home value
  {
    currentStatus.O2 = ADC_FILTER(145);
  }
  else
  {
    currentStatus.O2 = ADC_FILTER(tempO2value);
  }

#else
  // Read analog O2 value and convert it to AFR
  unsigned int tempReading;
  #if defined(ANALOG_ISR)
