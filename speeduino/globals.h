...
// Enable the next two lines to use the 14POINT7 SLC_OEM WBO via the I2C interface
//#define SLC_OEM_WBO
//#define SLC_OEM_WBO_I2C_ADDRESS 0x02
...
// The statuses that a schedule can have, used to save CPU cycle - we don't need to recalc ignition and fuel values more than one time
enum schedulingStatusEnum { ScheduleDataReady, TimerScheduled, TimerFired };

// Structure used to hold values required to schedule injectors/ignition
struct schedulingStruct {
  unsigned int start_tooth; // The tooth where we start the timer
  unsigned int start_delay_uS; // The wait time before the pulse is started after we have reached the right tooth
  unsigned int pulse_width_uS; // No need to explain this :-)
  unsigned int total_timer_time_uS; // start_delay_uS + pulse_width_uS used to speed up the interrupt handling if possible
  byte next_schedule; // This is used to define the next action see example below
  schedulingStatusEnum schedule_status;
  //byte outputPortMaskStart; // Could be used to optimise actions/save timers etc., added as a remark to remeber me on the option
  //byte outputPortMaskFinish; // Could be used to optimise actions/save timers etc., added as a remark to remeber me on the option
};

// Example for how next_schedule and cylider can be used
// The scheduler use this to select the next action
// 1. If we have one injector then injectorStruct[0].next_schedule should be set to 0
// 2. In a four cylinder with sequential injection order 1-3-4-2 the following would be true
// Dependend on if
//      injectorStruct[0].next_schedule = 1; //Cyl 1
//      injectorStruct[1].next_schedule = 2; //Cyl 3
//      injectorStruct[2].next_schedule = 3; //Cyl 4
//      injectorStruct[3].next_schedule = 0; //Cyl 2

// Injector timing array, one pr. injector needed
schedulingStruct injectorStruct[5];

// Ignition timing array, one pr. coil needed
 schedulingStruct ignitionStruct[5];

// This is the size of the array used starting at 0, would be 3 for a sequential 4 cylinder engine
byte injectorStructEnd = 0; // This is change in fuel_calc.ino
byte ignitionStructEnd = 0; // This is change in ignition_calc.ino

// This value is used in calculation for tooth scheduling - used in both fuel_calc.ino and ignition_calc.ino
// Value is set in decoders.ino
byte scheduling_max_tooth_count = 0;

// When data is calculated we need to start at the array number with the smallest tooth number
// Example:
// if injectorStruct[0].start_tooth < injectorStruct[injectorStructEnd].start_tooth then the number should be 0
// if injectorStruct[0].start_tooth > injectorStruct[injectorStructEnd].start_tooth then the number should be injectorStructEnd
byte injectorStructStart = 0;
byte ignitionStructStart = 0;

// Used in the timing proces called in decoders.ino at each tooth interrupt and in the ignition and fuel calc processes
struct schedule_struct {
  byte injectorNextScheduleTooth;
  byte ignitionNextScheduleTooth;
  byte injectionNextToSchedule;
  byte ignitionNextToSchedule;
} scheduleStatusStruct;
