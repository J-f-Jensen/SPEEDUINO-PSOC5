...
/*
The EEPROM in the CY8CKIT-059 PSOC5LP board only have 2KB EEPROM,
  based on this we use flash for the calibration tables
*/
#if defined(CORE_PSOC5)
  #include <em_eeprom.h>
#endif

//initialiseStorage components if required
void initialiseStorage()
{
#if defined(CORE_PSOC5)
  eeprom_initialize();
  em_eeprom_initialize();
...
*/
void UpdateStorage(int addr, uint8_t value)
{
...
#if defined(CORE_PSOC5)
  // PSOC5 don't like interupts while writing to EEPROM according to the datasheet
  noInterrupts();
  // Its recomented to update the temparture before writing on PSOC devices
...
{
#if defined(CORE_PSOC5)
    memcpy(cltCalibrationTable,EEPROM_CALIBRATION_CLT,512);
    memcpy(iatCalibrationTable,EEPROM_CALIBRATION_IAT,512);
    memcpy(o2CalibrationTable,EEPROM_CALIBRATION_O2,512);
...
#if defined(CORE_PSOC5)
  noInterrupts();
  // Save the memory tables to flash on a PSOC5 device
  em_eeprom_write(cltCalibrationTable,EEPROM_CALIBRATION_CLT,CALIBRATION_TABLE_SIZE);
  em_eeprom_write(iatCalibrationTable,EEPROM_CALIBRATION_IAT,CALIBRATION_TABLE_SIZE);
  em_eeprom_write(o2CalibrationTable,EEPROM_CALIBRATION_O2,CALIBRATION_TABLE_SIZE);
  interrupts();
