...
void initialiseTriggers()
{
  ...
  pinMode(pinTrigger3, INPUT);
      ...
      // Enable the injection/ignition timer(s)
  ...
  switch (configPage2.TrigPattern)
  {
    ...
    case 15:
      ...
      case 16:
            //Missing tooth decoder - Classic Mini SPI special - this require an secondary CAM trigger
            //due to that the tooth pattern is 36-2 with the missing tooths spaced by 180 degree
            triggerSetup_missingToothSPI();
            trigger = triggerPri_missingToothSPI;
            triggerSecondary = triggerSec_missingToothSPI;
            getRPM = getRPM_missingToothSPI;
            getCrankAngle = getCrankAngle_missingToothSPI;
            triggerSetEndTeeth = triggerSetEndTeeth_missingToothSPI;
            scheduling_max_tooth_count = 36;
            ...
            // A extra oneshoot timer + interupt is used in this decoder
            // Attach function to one shoot timer interupt
            CrankTriggerDummyOneShotTimer_attachinterupt(CrankTriggerDummyInterruptSPI);
            break;
...
// The next two internal status values are used to keep track on what the next schedule should be
// The first schedule will always be after tooth one
uint8_t doPerToothSchedulingNextInjection = 0;
uint8_t doPerToothSchedulingNextIgnition = 0;

// Defined in globals.h - date is calculated in fuel_calc.ino and ignition_calc.ino
extern schedulingStruct injectorStruct[];
extern schedulingStruct ignitionStruct[];

/*
Current only for PSOC5 implemntation, but it should be quit easy to create the same in the other micro processors
If we are at the defined start tooth-1 then the schedule is setup and enabled, at next raising tooth edge the scheduler
is started and when the pause time is over the pulse is executed. In the PSOC5 implemntation all after enabling is handled in hardware
*/
static inline void doPerToothScheduling( uint8_t dptsToothCurrentCount, uint8_t dptsRevolutionOne )
{
  // Ensure we is in sync - if pulse width for injector one is bigger than the time for one engine revolution this can be an issue.
  // We could reach this issue at pulse widths around 3mS at 10.000 RMP, if this is the case the bigger injectors is the way forward :-)
  if ( dptsToothCurrentCount == 1 && dptsRevolutionOne == 1 )
  {
    doPerToothSchedulingNextInjection = injectorStructStart;
    doPerToothSchedulingNextIgnition = ignitionStructStart;
  }

  // If the InjectionTimerControlRegister is 1 then an schedule is already running, we don't want to mess with this
  if ( InjectionTimerControlReg_1_Read() == 0 )
  {
    // If we are at the tooth before defined schedule start then we enable the schedule
    // Two timers are used for the injection part, otherwise we are limited to a very short pulse at high RMP
    if ( injectorStruct[doPerToothSchedulingNextInjection].start_tooth == dptsToothCurrentCount  && injectorStruct[doPerToothSchedulingNextInjection].pulse_width_uS != 0 )
    {
      // Setup the timer values
      InjectionTimer_1_WritePeriod( injectorStruct[doPerToothSchedulingNextInjection].total_timer_time_uS );
      InjectionTimer_1_WriteCompare( injectorStruct[doPerToothSchedulingNextInjection].pulse_width_uS );

      // Set the output pin
      InjectionTimerPortControlReg_1_Write( doPerToothSchedulingNextInjection );

      // Enable the scheduler
      InjectionTimerControlReg_1_Write( 1 );

      // Load the next injection schedule
      doPerToothSchedulingNextInjection = injectorStruct[doPerToothSchedulingNextInjection].next_schedule;
    }
  }
  else if ( InjectionTimerControlReg_2_Read() == 0 )
  {
    // If we are at the tooth before defined schedule start then we enable the schedule
    if ( injectorStruct[doPerToothSchedulingNextInjection].start_tooth == dptsToothCurrentCount && injectorStruct[doPerToothSchedulingNextInjection].pulse_width_uS != 0 )
    {
      // Setup the timer values
      InjectionTimer_2_WritePeriod( injectorStruct[doPerToothSchedulingNextInjection].total_timer_time_uS );
      InjectionTimer_2_WriteCompare( injectorStruct[doPerToothSchedulingNextInjection].pulse_width_uS );

      // Set the output pin
      InjectionTimerPortControlReg_2_Write( doPerToothSchedulingNextInjection );

      // Enable the scheduler
      InjectionTimerControlReg_2_Write( 1 );
  ...
  // If the IgnitionTimerControlRegister is 1 then an schedule is already running, we don't want to mess with this
  if ( IgnitionTimerControlReg_Read() == 0 )
  {
    // If we are at the tooth before defined schedule start then we enable the schedule
    if (( ignitionStruct[doPerToothSchedulingNextIgnition].start_tooth ) == dptsToothCurrentCount && ignitionStruct[doPerToothSchedulingNextInjection].pulse_width_uS != 0 )
    {
      // Setup the timer values
      IgnitionTimer_1_WritePeriod( ignitionStruct[doPerToothSchedulingNextIgnition].total_timer_time_uS );
      IgnitionTimer_1_WriteCompare( ignitionStruct[doPerToothSchedulingNextIgnition].pulse_width_uS );

      // Set the output pin
      IgnitionPortControlReg_Write( doPerToothSchedulingNextIgnition );

      // Enable the scheduler
      IgnitionTimerControlReg_Write( 1 );

      // Load the next ignition schedule
      doPerToothSchedulingNextIgnition = ignitionStruct[doPerToothSchedulingNextIgnition].next_schedule;
    }
  }
}
...
void triggerPri_missingTooth()
{
   ...
   if ( curGap >= triggerFilterTime ) //Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
   {
     ...
     {
#if defined(CORE_PSOC5)
        doPerToothScheduling(toothCurrentCount, revolutionOne);
#else
...
/*
Name: Missing tooth wheel - Classic Mini SPI (Rover MEMS)
!!! it require an secondary CAM trigger due to that the tooth pattern is 36-2 with the missing tooths spaced by 180 degree
Desc: A multi-tooth wheel with one of more 'missing' teeth. The first tooth after the missing one is considered number 1 and is the basis for the trigger angle
*/
void triggerSetup_missingToothSPI()
{
  triggerToothAngle = 360 / 36; //The number of degrees that passes from tooth to tooth
  triggerActualTeeth = 17; //The number of physical teeth on the wheel. Doing this here saves us a calculation each time in the interrupt
...
uint8_t secondaryTriggerStatusSPI = 0;
uint8_t realtoothCurrentCount = 0;

uint16_t curGapTMP = 0;

void triggerPri_missingToothSPI()
{
   // http://www.msextra.com/forums/viewtopic.php?f=94&t=22976
   // http://www.megamanual.com/ms2/wheel.htm
   ...
   if ( curGap >= triggerFilterTime ) //Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
   {
     ...
     realtoothCurrentCount++; //Increment the real physical tooth Counter

     addToothLogEntry(curGap);

     // An extra timer is used to emulate the missing tooth(s)
     // this makes tooth based ignition and fuel calculations simpler and more acurate
     if ( realtoothCurrentCount == 17 )
     {
       curGapTMP = curGap - 10; // This time should be adjusted more exactly - but 10 uS is quit close so I will look at that later
       CrankTriggerDummyOneShotTimer_reload( curGapTMP );
     }
     ...
     if ( (curGap > targetGap) || (realtoothCurrentCount > triggerActualTeeth) )
     {
       if( (realtoothCurrentCount < triggerActualTeeth) && (currentStatus.hasSync == true) )
       {
        //This occurs when we're at tooth #1, but haven't seen all the other teeth. This indicates a signal issue so we flag lost sync so this will attempt to resync on the next revolution.
         currentStatus.hasSync = false;
         // for the SPI 36-2 tooth wheel we need to see the secondary trigger before we know where we are
         secondaryTriggerStatusSPI = 0;
       }
       //This is to handle a special case on startup where sync can be obtained and the system immediately thinks the revs have jumped:
       //else if (currentStatus.hasSync == false && toothCurrentCount < checkSyncToothCount ) { triggerFilterTime = 0; }
       else
       {
         if ( secondaryTriggerStatusSPI != 0 || secondaryTriggerStatusSPI != 6 )
         {
           realtoothCurrentCount = 1;

           if ( secondaryTriggerStatusSPI == 1 ||  secondaryTriggerStatusSPI == 3)
           {
             toothCurrentCount = 1;
             revolutionOne = !revolutionOne; //Flip sequential revolution tracker
             currentStatus.startRevolutions++; //Counter
           ...
           currentStatus.hasSync = true;
           triggerFilterTime = 0; //This is used to prevent a condition where serious intermitent signals (Eg someone furiously plugging the sensor wire in and out) can leave the filter in an unrecoverable state
           secondaryTriggerStatusSPI++;
         }
         else  // We should not hit this when the engine is running, only at start until we have secondaryTrigger sync
         {
     ...
     // Start pr tooth schedule
     doPerToothScheduling( toothCurrentCount, revolutionOne );
   }
}

void triggerSec_missingToothSPI()
{
  //TODO: This should really have filtering enabled on the secondary input.
  revolutionOne = 0; // Set to 0 due to that it is flipped in the triggerPri.... function at first tooth
  secondaryTriggerStatusSPI = 1;
}

uint16_t getRPM_missingToothSPI()
{
  uint16_t tempRPM = 0;
  if( (currentStatus.RPM < currentStatus.crankRPM) && (realtoothCurrentCount != 1) ) //Can't do per tooth RPM if we're at tooth #1 as the missing tooth messes the calculation
  {
...
int getCrankAngle_missingToothSPI(int timePerDegree)
{
    // Not used in the special PSOC5 tooth based scheduling
    return 180;
}

void triggerSetEndTeeth_missingToothSPI()
{
  // Not used in the PSOC5 tooth based scheduling
}

// Handle the dummy tooth interrupt
void CrankTriggerDummyInterruptSPI()
{
  // Not in sync -> nothing to do here lets return
  if ( toothCurrentCount == 0 ) { return; }

  // Increase the tooth counter
  toothCurrentCount++;

  // Run the pr tooth schedule process
  doPerToothScheduling(toothCurrentCount,0);
}
