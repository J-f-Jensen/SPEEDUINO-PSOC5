...
void command()
{
  ...
  switch (currentCommand)
  {
    ...
    case 'd': // Send data version from eeprom
      Serial.print(ReadStorage(EEPROM_DATA_VERSION));
      break;
    ...
    */
    case 'p':
      ...
      if(Serial.available() >= 6)
      {
        ...
        // Check if we have recived bad data
        if (length > 288)
        {
          Serial.write(length1);
          Serial.write(length2);
          Serial.write(length);
          break;
        }

        //Create buffer
        uint8_t pBuffer[length];

        //Read data into buffer
        for(int i = 0; i < length; i++)
        {
            pBuffer[i] = getPageValue(tempPage, valueOffset + i);
        }

        //Send data
        Serial.write(pBuffer, length);
    ...
    case 'Q': // send code version
      Serial.print("speeduino 201801-PSOC5");
      break;
    ...
    case 'S': // send code version
      Serial.print("Speeduino 2018.01-PSOC5");
    ...
    case '?':
      ...
      (F(
         ...
         "c - Send the current loops/sec value.\n"
         "L - List the contents of current page in human readable form.\n"
         "N - Print new line.\n"
  ...
  switch (currentPage)
  {
    ...
    case veSetPage:
        if (useChar)
        {
          ...
          Serial.println((const __FlashStringHelper *)&pageTitles[9]);//9 is the index to the first char in the second sting in pageTitles
    ...
    case ignMapPage:
        currentTitleIndex = 22;// the index to the first char of the third string in pageTitles
    ...
    case ignSetPage:
        if (useChar)
        {
          ...
          Serial.println((const __FlashStringHelper *)&pageTitles[36]);
    ...
    case afrMapPage:
        currentTitleIndex = 49;//Array index to next string
    ...
    case afrSetPage:
        if (useChar)
        {
          ...
          Serial.println((const __FlashStringHelper *)&pageTitles[58]);//special typecasting to enable suroutine that the F macro uses
        ...
        if (useChar)
        {
          Serial.println((const __FlashStringHelper *)&pageTitles[71]);// F macro hack
    ...
    case canbusPage:
        if (useChar)
        {
          ...
          Serial.println((const __FlashStringHelper *)&pageTitles[104]);//special typecasting to enable suroutine that the F macro uses
          for (pnt_configPage = &configPage10; pnt_configPage < ((byte *)pnt_configPage + 128); pnt_configPage = (byte *)pnt_configPage + 1)
          {
            Serial.write(*((byte *)pnt_configPage));// Displaying byte values of config page 3 up to but not including the first array
          }
          sendComplete = true;
        }
...
void receiveCalibration(byte tableID)
{
  ...
  switch (tableID)
  {
    case 0:
      ...
      DIVISION_FACTOR = 10;
      BYTES_PER_VALUE = 2;
      EEPROM_START = (int)EEPROM_CALIBRATION_CLT;
      break;
    case 1:
      ...
      DIVISION_FACTOR = 10;
      BYTES_PER_VALUE = 2;
      EEPROM_START = (int)EEPROM_CALIBRATION_IAT;
      break;
    case 2:
      //O2 table
      ...
      DIVISION_FACTOR = 1;
      BYTES_PER_VALUE = 1;
      EEPROM_START = (int)EEPROM_CALIBRATION_O2;
      break;
  ...
  for (x = 0; x < 1024; x++)
  {
    ...
    if (every2nd) //Only use every 2nd value
    {
...
#ifndef CORE_PSOC5 // Don't look like a problem at PSOC5 devices
      ...
      #elif defined(CORE_PSOC5) // Do nothing
      #else
