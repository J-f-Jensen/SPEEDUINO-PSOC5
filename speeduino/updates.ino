...
void doUpdates()
{
  ...
  if(ReadStorage(EEPROM_DATA_VERSION) == 2)
  {
    for(int x=0; x<16; x++)
    {
      for(int y=0; y<16; y++)
  ...
  if(ReadStorage(EEPROM_DATA_VERSION) == 4)
  {
    ...
    writeAllConfig();
    UpdateStorage(EEPROM_DATA_VERSION, 5);
  }
  ...
  if(ReadStorage(EEPROM_DATA_VERSION) == 5)
  {
    ...
    for(int x=0; x < 1152; x++)
    {
      ...
      byte currentVal = ReadStorage(startMem);
      UpdateStorage(endMem, currentVal);
    }
    ...
    UpdateStorage(EEPROM_DATA_VERSION, 6);
    loadConfig(); //Reload the config after changing everything in EEPROM
  }

  //Final check is always for 255 and 0 (Brand new arduino)
  if( (ReadStorage(EEPROM_DATA_VERSION) == 0) || (ReadStorage(EEPROM_DATA_VERSION) == 255) )
  {
    configPage10.true_address = 0x200;
    UpdateStorage(EEPROM_DATA_VERSION, CURRENT_DATA_VERSION);
  }

  //Check to see if someone has downgraded versions:
  if( ReadStorage(EEPROM_DATA_VERSION) > CURRENT_DATA_VERSION ) { UpdateStorage(EEPROM_DATA_VERSION, CURRENT_DATA_VERSION); }
}
